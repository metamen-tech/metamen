...
export function calculateOverallScore(vectors: VectorState): Result<number> {
  if (!isValidVectorState(vectors))
    return err(new Error('Invalid vector state'));

  const score =
    vectors.aura * 0.20 +
    vectors.jawline * 0.15 +
    vectors.wealth * 0.20 +
    vectors.physique * 0.20 +
    vectors.social * 0.15 +
    (vectors.env * 5) * 0.10;
...
export function calculateBtcMultiplier(
  ...
  const subBonus = hasSubscription ? 0.20 : 0;
...
export function calculateDiminishingReturns(
  ...
  const multiplier = Math.max(0.25, Math.pow(0.90, repetitionCount));
...
export function calculateDeathPenalty(
  ...
  const btcLossRate = deathCount === 0 ? 0.30 : deathCount === 1 ? 0.40 : 0.50;

  return {
    btcLoss: Math.floor(currentBtc * btcLossRate),
    newAura: Math.round(currentAura * 0.30 * 100) / 100,
...
| DB indexes | idx_ prefix | `idx_profiles_user_id` |
